...
namespace Crap4n.FileParser
{
    public class SourceMonitorFileParser : IFileParser<CodeMetrics>
    {
        ...
        private CodeMetrics CreateCodeMetricsForMethodFromXml(XmlNode methodMetrics)
        {
            ...
            var codeMetrics = new CodeMetrics
                                  {
                                      Class = classAndName[0],
                                      SourceFile = GetSourceFile(methodMetrics),
                                      SourceFileLineNumber = GetSourceFileLineNumber(methodMetrics)
                                  };
            if (classAndName.Length == 3)
            {
                if (IsProperty(classAndName))
                    codeMetrics.Method = GetPropertyName(classAndName);
                        //Explicit interface with property is bugged in SourceMonitor
                else
